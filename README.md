...
- **Data.** Supabase Postgres + pgvector (768-dim). RLS enabled with
  permissive demo policies; `match_*` RPCs for semantic recall.
- **AI.** Google Gemini — `gemini-2.5-flash` for extraction and workspace
...
**Fastest path — paste into the dashboard SQL editor:**

1. Supabase dashboard → **Database → Extensions** → search `vector` → enable.
2. Dashboard → **SQL Editor** → New query → paste the full contents of
   `supabase/migrations/20260418140000_init.sql` → **Run**.
...
supabase link --project-ref YOUR_PROJECT_REF       # asks for DB password once
supabase db push                                   # apply the migration
psql "$POOLER_URI" -f supabase/seed.sql            # apply demo seed
```

If you previously applied the migration with 1536-dim vectors (old
Anthropic/OpenAI version), wipe and reapply — the vector column size must
...
src/
  ...
  lib/
    ...
    ai/                 gemini client, extract, rollup, embed
    queries/            workspaces, signals, decisions, timeline, ideas, recent, health
supabase/
  migrations/20260418140000_init.sql
  seed.sql
