...
export async function POST(
  ...
  if (signal.workspace_id !== workspaceId) {
    return Response.json({ success: false, error: "Signal does not belong to workspace" }, { status: 400 });
  }
  ...
  if (saveError || !saved) {
    return Response.json({ success: false, error: "Failed to save canonical memory" }, { status: 500 });
  }
