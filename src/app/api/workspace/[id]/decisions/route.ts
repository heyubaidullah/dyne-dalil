...
const requestSchema = z.object({
  ...
  signal_ids: z.array(z.string().uuid()).max(20).optional(),
});
...
export async function POST(
  ...
  if (payload.signal_ids && payload.signal_ids.length > 0) {
    await supabase.from("decision_evidence").insert(
      payload.signal_ids.map((sid) => ({
  ...
  // Seed a pending outcome so timelines show the decision tracking.
